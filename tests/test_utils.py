...
def test_call_wind_map_api():
    ...
    assert np.allclose(new_gdf_newa.loc[1][expected_column_name], 9.077987670898438)
    ...
    assert isinstance(new_gdf_newa.loc[2][expected_column_name], xr.DataArray)
    assert np.allclose(new_gdf_newa.loc[2][expected_column_name].values[0][0], 7.2537737)
    assert "Invalid geometry type" in new_gdf_newa.loc[3][expected_column_name]

    # add newa-microscale tests once API back up 
