...
1. Added the option to return a table of device orientation offsets (`return_direction_offset_summary`) to `apply_device_orientation_offset`. This provides the offset in the logger and that applied in the analysis for each relevant time period ([#521](https://github.com/brightwind-dev/brightwind/issues/521)).
