...
def test_extrapolate_density():
    assert bw.extrapolate_density(reference_density_kg_m3=1.224, reference_height_m=80, target_height_m=100) == 1.22174
    assert bw.extrapolate_density(reference_density_kg_m3=1.224, reference_height_m=80, target_height_m=100,lapse_rate=-0.0002,print_details=True) == 1.22
    test_data = DATA.loc['2016-01-09 17:10':'2016-01-09 18:00']
    test_density = bw.calc_air_density(test_data.T2m, test_data.P2m)
    pd.testing.assert_series_equal(bw.extrapolate_density(reference_density_kg_m3=test_density, reference_height_m=2, target_height_m=10), pd.Series(data = [1.18678, 1.18717, 1.18775, 1.18595, 1.1863 , 1.18569],
...
def test_extrapolate_temperature():
    assert bw.extrapolate_temperature(10.0065, 10, 11, print_details=True) == 10.0
    assert bw.extrapolate_temperature(DATA.T2m.loc['2016-01-09 17:10'], 4, 2, lapse_rate= -0.005)
    pd.testing.assert_series_equal(bw.extrapolate_temperature(DATA.T2m.loc['2016-01-09 17:10':'2016-01-09 18:00'], 2, 20, print_details=True),
